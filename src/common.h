...
constexpr long long MASK = (1LL << 20);
...
inline std::pair<int, int> unpack_fast_pair(long long fp) {
	return std::make_pair(static_cast<int>(fp / MASK), static_cast<int>(fp % MASK));
}

inline long long make_fast_triple(int a, int b, int c) { // assuming a >= 0 && b >= 0 && c >= 0
	return a * MASK * MASK + b * MASK + c;
}
...
inline std::tuple<int, int, int> unpack_fast_triple(long long ft) {
	return std::make_tuple(
			static_cast<int>(ft / (MASK * MASK)),
			static_cast<int>(ft % (MASK * MASK) / MASK),
