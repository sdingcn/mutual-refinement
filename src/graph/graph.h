...
struct Graph {
	...
	std::vector<long long> runCFLReachability(const Grammar &grammar);
	std::unordered_set<long long> getEdgeClosure(const Grammar &grammar, const std::vector<long long> &startSummaries) const;
	void clear();
};
